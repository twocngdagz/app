...
(function ($) {
		...
		p = $.extend({ //apply default properties
			...
			minheight: 90, //min height of columns
		...
		var g = {
			...
			dragEnd: function () {
				if (this.colresize) {
					...
					$(this.colCopy).remove();
					if (this.dcolt != null) {
						if(p.checkbox){if(this.dcolt==0){x=false;}}
						if(x){
			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						if(p.onChange){
							$(tr).click(function () {p.onChange(row.id);});
						}							
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								var a = true;
								if(p.checkbox){idx=idx-1;}
								if(idx!=-1){	
  										if(p.colModel[idx].input){										
											inp.className = p.colModel[idx].class;
											inp.id = row.id;
											inp.type = p.colModel[idx].type;
											inp.onpress = p.colModel[idx].onpress;
											if(p.colModel[idx].readonly == true){
												inp.setAttribute("readonly",'true');
											}
											inp.setAttribute("value",row.cell[idx]);
											$(td).append(inp);
											//inp=null;	

										}
										if(p.colModel[idx].class){

											if(row.cell[idx].length>0)$(td).addClass(p.colModel[idx].class);
										}
									
										if(td.innerHTML==""){
											td.id = p.colModel[idx].name + row.id;
										
											if (typeof row.cell[idx] != "undefined") {
												td.innerHTML = row.cell[idx];
											} else {
												td.innerHTML =p.colModel[idx].name;
											}
										}
										if(p.colModel[idx].context){
																			
												for (x = 0; x < p.colModel[idx].context.length; x++) {
												var divimg = document.createElement('span');
								
												divimg.innerHTML = "<span class='fc"+ row.id+"' onclick='flexcontext("+row.id+","+p.colModel[idx].context[x][1]+")'style='display:none;cursor:pointer;width:20px;float:right;background:url("+p.colModel[idx].context[x][0]+") no-repeat center');>&nbsp</span>";
												$(td).append(divimg);
												 divimg = null;
												}
												$(td).hover(function () {$(".fc"+row.id).show();});
												$(td).mouseleave(function () {$(".fc"+row.id).hide();});

										}
										if(p.colModel[idx].onpress){
												td.id = row.id;
												if(row.cell[idx].length>0)$(td).click(function () {p.colModel[idx].onpress(row.id);});
										}
										if(p.colModel[idx].condition){
												for (x = 0; x < p.colModel[idx].condition.length; x++) {
													
													if( row.cell[idx] ==  p.colModel[idx].condition[x][0]){
														$(td).css('background-color', p.colModel[idx].condition[x][1]);
													}
												}
				...
				if(p.checkbox){
					$('#chkAll').each(function(){
						$(this).click(function(){
							if($('#chkAll').is(':checked')){
									$('tbody input',g.bDiv).attr('checked',true);
									$('tbody tr',g.bDiv).addClass('trSelected');
								} else {
									$('tbody :checked',g.bDiv).attr('checked',false);
					...
					$('tbody :input').each(function(){
						$(this).change(function(){
							var rowid = '#row'+this.name;
							if($(rowid).attr('class')){
							$(rowid, g.bDiv).removeClass('trSelected');
							} else {
							$(rowid, g.bDiv).addClass('trSelected');}
		...
		if (p.colModel) { //create model if any
			...
			if(p.checkbox){
				...
				th.innerHTML="<input id='chkAll' type='checkbox'/>";
				$(th).attr('axis', 'col');
				th.align = 'center';
				th.id = 'chkboxHead';
				$(th).attr('width',20);
		...
		$('thead tr:first th', g.hDiv).each(function () {
			...
			$(this).empty().append(thdiv).removeAttr('width').mousedown(function (e) {
				...
				if (!g.colresize && !$(this).hasClass('thMove') && !g.colCopy) {					
					if(!this.fix){$(this).addClass('thOver')};
				}
		...
		if (p.usepager) {
			...
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <div class="btnseparator"></div><input type="text" style="width:20px" size="1" value="1" /> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
	...
	$.fn.flexRequery = function(p,s){
	
		return this.each(function(){
			this.p.query = p; this.p.qtype = s;
