...
class MySQL
{
	...
	public function __construct($connect = true, $database = null, $server = null,
		...
		$this->db_dbname 	= $Settings->dbsuffix."_".$Settings->dbname;
		$this->db_user		= $Settings->dbusername;
		$this->db_pass 		= $Settings->dbpass;
		$this->db_suffix 	= $Settings->dbsuffix;
	...
	function getPersonID(){
		...
		if(!$_COOKIE["__amsuin"]) return false;	
		$values["sessionid"] =MySQL::SQLValue($_COOKIE["__amstoken"]);
			
		$this->Query(MySQL::BuildSQLSelect($this->db_suffix."_loginsessions", $values));
		$val = $this->Row();
		$values= array();
		$values["userid"] = MySQL::SQLValue($val->userid,MySQL::SQLVALUE_NUMBER);
			
		$this->Query(MySQL::BuildSQLSelect($this->db_suffix."_users", $values));
		return $this->Row()->personid;
		
	}
	function getPrefix(){
		$Settings = New Settings();
		return $Settings->dbsuffix;
	}
	function Login(){
			require_once("jcryption.php");
			if(!$_COOKIE["__amsuin"]) return false;
			...
			$val = $this->Row();
			$values= array();
			$values["username"] = MySQL::SQLValue(AesCtr::decrypt($_COOKIE["__amsuin"] ,$val->rsakey, 256));
			$values["password"] = MySQL::SQLValue(md5(AesCtr::decrypt($_COOKIE["__amsuip"] ,$val->rsakey, 256)));
			
			if(!$this->HasRecords(MySQL::BuildSQLSelect($this->db_suffix."_users", $values))){
					return false;
			} else {
					return AesCtr::decrypt($_COOKIE["__amsuin"] ,$val->rsakey, 256);
			}
	}
	function translate($str)
	{
		require_once("jcryption.php");
		if(!isset($_COOKIE["__amstoken"]))return false;
		$values["sessionid"] =MySQL::SQLValue($_COOKIE["__amstoken"]);
		...
		$this->Query(MySQL::BuildSQLSelect($this->db_suffix."_users", $values));
		$val = $this->Row();
		return  AesCtr::decrypt($str,$val->rsakey, 256);
		} else return false;
	}
	function LoginAdmin($username,$password){
			$values["username"] = MySQL::SQLValue($username);
			$values["password"] = MySQL::SQLValue($password);
			$values["userrole"] = MySQL::SQLValue(-1,MySQL::SQLVALUE_NUMBER);
			if(!$this->HasRecords(MySQL::BuildSQLSelect($this->db_suffix."_users", $values))){
					return false;
			} else {
					return true;
	...
	function checkaccess($pageid,$access){
			if(!$this->Login()) exit();
			require_once("jcryption.php");
			if(!$_COOKIE["__amsuin"]) return false;
			...
			$this->Query(MySQL::BuildSQLSelect($this->db_suffix."_users", $values));
			$accesslevel = $this->Row();
			
			if($accesslevel->superadmin == 1) return true;
			
			
			$filter["personid"]= $accesslevel->personid;
			$groups = $this->SelectRows($this->db_suffix."_roles",$filter);
			while ($row = mysql_fetch_assoc($groups)) {
				if($this->getParentAccess($pageid,$access,$row->groupid)) {
					return true;
				}
				$this->Query("SELECT ".$this->getPrefix()."_accesssettings.".$access." FROM ".$this->getPrefix()."_accesssettings WHERE groupid=".$row->groupid." AND itemid=".$pageid);
				$value2= $this->Row();
				if($value2->$access == 1) { 
					return true;
				}
	...
	function getParentAccess($pageid,$access,$accesslevel){
			$this->Query("SELECT parentid FROM ".$this->getPrefix()."_accessitems WHERE itemid=".$pageid);
			$value = mysql_fetch_object($this->last_result);
			if($value->parentid == null || $access == 'view' ){
				$this->Query("SELECT ".$this->getPrefix()."_accesssettings.".$access." FROM ".$this->getPrefix()."_accesssettings WHERE groupid=".$accesslevel." AND itemid=".$pageid);
				$value2= $this->Row();
				if($value2->$access == 1) { 
					return true;
				}
			} else {
				if($this->getParentAccess($value->parentid,$access,$accesslevel)){
					return true;
				} else {
					
					$this->Query("SELECT ".$this->getPrefix()."_accesssettings.".$access." FROM ".$this->getPrefix()."_accesssettings WHERE groupid=".$accesslevel." AND itemid=".$pageid);
					$value2= mysql_fetch_object($this->last_result);
					if($value2->$access == 1) {
						return true;
					}
				}
	...
	function changeConfig($dbhost,$dbuser,$dbpass,$dbname,$dbsuffix){
			$handle = fopen("../includes/config/dbconfig.php", "w");
			$string = "<?php \n".
				"$"."Settings = new Settings();\n".
				"class Settings{ \n".
				"var $"."dbhost  = '".$dbhost."';\n".
	...
	*/
	public function GetFlexiJson($sql='',$tb='',$where='',$sort='',$limit='',$page=0,$total=0){
		$this->query("SELECT SQL_NO_CACHE Count(*) as X FROM $tb $where $sort");
		$row = $this->Row();
		$total = $row->X;
		$this->query("$sql $tb $where $sort $limit");
		...
		$rc = false;
		$this->MoveFirst();
		while ($member = mysql_fetch_row($this->last_result)) {
			if ($rc) $json .= ",";
			$json .= "\n{";
			$json .= '"id":'.$member[0].',';
			$json .= '"cell":[';
	 ...
	 *
	 * @return object PHP 'mysql result' resource obtmlject containing the records
